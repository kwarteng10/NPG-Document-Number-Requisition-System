...
def show_request_page():
    ...
    if Request:
        ...
        if not st.session_state.title:
            ...
            cursor.execute("INSERT INTO spreadsheet (Date, Requester,Title,Department, Doc_Type, Doc_Num) VALUES (?,?,?,?,?,?)", (date,st.session_state.email, st.session_state.title,department,document_code,document_num))
            conn.commit()
